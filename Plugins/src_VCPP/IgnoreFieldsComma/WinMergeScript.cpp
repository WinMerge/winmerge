...
{
	*pVal = SysAllocString(L"Ignore fields - list from plugin name/argument");
	return S_OK;
}
...
STDMETHODIMP CWinMergeScript::PrediffBufferW(BSTR *pText, INT *pSize, VARIANT_BOOL *pbChanged, VARIANT_BOOL *pbHandled)
{
	...
	const WCHAR delimiter = MergeApp_GetOptionString(m_pMergeApp, _T("Settings/CSVDelimiterCharacter"), _T(","))[0];
