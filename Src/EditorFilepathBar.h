...
public:
	...
	virtual void SetOnGetRecentItemsCallback(const std::function<std::vector<HistoryItemsHelper::RecentItem>(int pane, unsigned maxCount, HistoryItemsHelper::RecentItemType type)> callbackfunc) = 0;
	virtual void SetOnGetClipboardHistoryCallback(const std::function<std::vector<HistoryItemsHelper::ClipboardItem>(unsigned maxCount)> callbackfunc) = 0;
};
...
// Dialog Data
	...
	void SetOnGetRecentItemsCallback(const std::function<std::vector<HistoryItemsHelper::RecentItem>(int pane, unsigned maxCount, HistoryItemsHelper::RecentItemType type)> callbackfunc) override;
	void SetOnGetClipboardHistoryCallback(const std::function<std::vector<HistoryItemsHelper::ClipboardItem>(unsigned maxCount)> callbackfunc) override;
...
private:
	...
	std::vector<HistoryItemsHelper::RecentItem> GetRecentItems(int pane, unsigned maxCount, HistoryItemsHelper::RecentItemType type = HistoryItemsHelper::RecentItemType::All) const;
	std::vector<HistoryItemsHelper::ClipboardItem> GetClipboardHistory(unsigned maxCount) const;
	...
	std::function<std::vector<HistoryItemsHelper::RecentItem>(int pane, unsigned maxCount, HistoryItemsHelper::RecentItemType type)> m_getRecentItemsCallbackfunc;
	std::function<std::vector<HistoryItemsHelper::ClipboardItem>(unsigned maxCount)> m_getClipboardHistoryCallbackfunc;
	std::vector<std::shared_ptr<TempFile>> m_tempFiles; /**< Temp files from clipboard history */
	std::vector<HistoryItemsHelper::ClipboardItem> m_cachedClipboardItems; /**< Cached clipboard items for menu consistency */
};
