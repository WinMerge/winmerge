...
PropCompare::PropCompare(COptionsMgr *optionsMgr) 
 ...
 , m_bIgnoreEofNewlinePresence(true)
 , m_nIgnoreWhite(-1)
...
void PropCompare::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_IGNEOFEOL_CHECK, m_bIgnoreEofNewlinePresence);
...
void PropCompare::ReadOptions()
{
	...
	m_bIgnoreEofNewlinePresence = GetOptionsMgr()->GetBool(OPT_CMP_IGNORE_EOF_NEWLINE_PRESENCE);
...
void PropCompare::WriteOptions()
{
	...
	GetOptionsMgr()->SaveOption(OPT_CMP_IGNORE_EOF_NEWLINE_PRESENCE, m_bIgnoreEofNewlinePresence);
...
void PropCompare::OnDefaults()
{
	...
	m_bIgnoreEofNewlinePresence = GetOptionsMgr()->GetDefault<bool>(OPT_CMP_IGNORE_EOF_NEWLINE_PRESENCE);
