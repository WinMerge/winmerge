...
bool storageForPlugins::SaveAsFile(String& filename)
{
	try
	{
		// If the current data is already a file, use it as-is.
		// The file may have been generated by a plugin and its actual
		// encoding is not necessarily represented by m_codepage.
		if (m_bCurrentIsFile)
		{
			filename = m_filename;
			return true;
		...
		// Save using the original file encoding.
		const auto& encoding = m_fileEncoding;

		// UTF-32 is currently not supported by CrossConvert/FileTextEncoding.
		...
		// For an ANSI buffer, no conversion is required when the target
		// encoding is also the current ANSI codepage.
		...
		// Allocate enough space for the converted data.
		//
		// CrossConvert() reserves one byte for the terminating NUL,
		// so add one extra byte here.
		int textForeseenSize;
		if (m_bCurrentIsUnicode)
			textForeseenSize = static_cast<int>(nchars * 3) + 1;
		else
			textForeseenSize = static_cast<int>(nchars) + 1;
...

		{
			...
			{
				// Preserve arbitrary bytes exactly.
		...
		// Shrink the file to the actual size.
