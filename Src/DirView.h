...
protected:
	...
	afx_msg void OnViewShowAllItems();
	afx_msg void OnViewShowDiffItems();
	afx_msg void OnViewShowSameItems();
	afx_msg void OnUpdateViewShowPreset(CCmdUI* pCmdUI);
...
private:
	...
	bool IsSplitTreeActive() const;
	eExpandSubfoldersType GetExpandSubdirsSetting() const;
	void EnsureSplitPaneTreeMode();
	int GetSplitTreeNamePane(int physCol) const;
	int GetTreeIndentPx() const;
	CRect GetItemCellRect(int nRow, int nCol) const;
	bool ShouldDrawSplitTreeExpander(int nRow, int pane) const;
	CRect GetSplitTreeExpanderRect(int nRow, int pane, const CRect& cell) const;
	bool HitTestSplitTreeExpander(int nRow, int pane, CPoint pt) const;
	void DrawSplitTreeNameCell(NMLVCUSTOMDRAW* lpC, int pane);
	void DrawSplitTreeLines(CDC* pDC, int nRow, int pane, const CRect& cell, int indentPx, int expanderSize);
	int GetDirViewShowPreset() const;
	void ApplyDirViewShowPreset(int preset);
