...
//  REXX keywords
static const tchar_t * s_apszRexxKeywordList[] =
  {
    _T ("ADDRESS"),
    _T ("ARG"),
    _T ("BY"),
...
static bool
IsRexxKeyword (const tchar_t *pszChars, int nLength)
{
...
unsigned
CrystalLineParser::ParseLineRexx (unsigned dwCookie, int nLineIndex, const tchar_t *pszChars, int nLength, TEXTBLOCK * pBuf, int &nActualItems, void *pContext)
{
  if (nLength == 0)
