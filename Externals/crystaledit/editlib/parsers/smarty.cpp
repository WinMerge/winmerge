...
static inline void
DefineIdentiferBlock(const tchar_t *pszChars, int nLength, std::vector<CrystalLineParser::TEXTBLOCK>& blocks, int nIdentBegin, int I, DWORD dwCookie)
{
...
unsigned
CrystalLineParser::ParseLineSmarty(unsigned dwCookie, const tchar_t* pszChars, int nLength, std::vector<TEXTBLOCK>& blocks)
{
  return ParseLineHtmlEx(dwCookie, pszChars, nLength, blocks, SRC_SMARTY);
}

unsigned
CrystalLineParser::ParseLineSmartyLanguage (unsigned dwCookie, const tchar_t *pszChars, int nLength, std::vector<TEXTBLOCK>& blocks)
{
  if (nLength == 0)
