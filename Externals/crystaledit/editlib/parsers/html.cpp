...
unsigned
CrystalLineParser::ParseLineHtmlEx (unsigned dwCookie, int nLineIndex, const tchar_t *pszChars, int nLength, TEXTBLOCK * pBuf, int &nActualItems, int nEmbeddedLanguage, void *pContext)
{
  if (nLength == 0)
    {
          ...
          if (dwCookie & COOKIE_BLOCK_SCRIPT)
            {
              ...
              dwCookie = ParseLineJavaScript(dwCookie & ~COOKIE_BLOCK_SCRIPT, nLineIndex, pszChars + I, nextI - I, pBuf + nActualItems, nActualItemsEmbedded, pContext);
          ...
          else if (dwCookie & COOKIE_BLOCK_STYLE)
            {
              ...
              int nextI = pszEnd ? static_cast<int>(pszEnd - pszChars) : nLength;
              int nActualItemsEmbedded = 0;
              dwCookie = ParseLineCss(dwCookie & ~COOKIE_BLOCK_STYLE, nLineIndex, pszChars + I, nextI - I, pBuf + nActualItems, nActualItemsEmbedded, pContext);
              AdjustCharPosInTextBlocks(pBuf, nActualItems, nActualItems + nActualItemsEmbedded - 1, I);
              nActualItems += nActualItemsEmbedded;
              if (!pszEnd)
              ...
              if (nEmbeddedLanguage != SRC_SMARTY)
                {
                  ...
                  int nextI = pszEnd ? static_cast<int>(pszEnd - pszChars) : nLength;
                  unsigned (*pParseLineFunc)(unsigned, int, const tchar_t *, int, TEXTBLOCK *, int &, void *);
                  ...
                  int nActualItemsEmbedded = 0;
                  dwCookie = pParseLineFunc(dwCookie & ~COOKIE_EXT_USER1, nLineIndex, pszChars + I, nextI - I, pBuf + nActualItems, nActualItemsEmbedded, pContext);
                  AdjustCharPosInTextBlocks(pBuf, nActualItems, nActualItems + nActualItemsEmbedded - 1, I);
                  nActualItems += nActualItemsEmbedded;
                  if (!pszEnd)
                    {
              ...
              else
                {
                  ...
                  dwCookie = ParseLineSmartyLanguage(dwCookie & ~COOKIE_EXT_USER1, nLineIndex, pszChars + nOffset, nextI - nOffset + 1, pBuf + nActualItems, nActualItemsEmbedded, pContext);
...
unsigned
CrystalLineParser::ParseLineHtml (unsigned dwCookie, int nLineIndex, const tchar_t *pszChars, int nLength, TEXTBLOCK * pBuf, int &nActualItems, void *pContext)
{
  return ParseLineHtmlEx(dwCookie, nLineIndex, pszChars, nLength, pBuf, nActualItems, SRC_JAVA, pContext);
}
