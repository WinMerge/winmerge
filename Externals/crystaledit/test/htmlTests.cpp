		...
		TEST_METHOD(HTMLSyntax)
		{
			...
			for (size_t i = 0; i < std::size(expected); ++i)
			{
				...
				Assert::AreEqual(
					...
					CrystalLineParser::ParseLineHtml(data[i].dwCookie, i, data[i].pszChars, static_cast<int>(tc::tcslen(data[i].pszChars)), pblocks.get(), nActualItems, nullptr), msg.c_str());
