		...
		TEST_METHOD(BatchSyntax)
		{
			...
			for (size_t i = 0; i < std::size(expected); ++i)
			{
				...
				Assert::AreEqual(
					...
					CrystalLineParser::ParseLineBatch(data[i].dwCookie, i, data[i].pszChars, static_cast<int>(tc::tcslen(data[i].pszChars)), pblocks.get(), nActualItems, nullptr), msg.c_str());
